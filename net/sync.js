...
export function initNetSyncSystem() {
    EM.addSystem("netSync", Phase.NETWORK, [AuthorityDef, SyncDef], [TimeDef, MeDef], (ents, res) => {
...
export function initNetUpdateSystems() {
    EM.addSystem("clearRemoteUpdatesMarker", Phase.NETWORK, [RemoteUpdatesDef], [], (es) => {
        for (const e of es) {
            EM.removeComponent(e.id, RemoteUpdatesDef);
        }
    });
    EM.addSystem("netUpdate", Phase.NETWORK, [PeerDef, InboxDef, OutboxDef], [TimeDef, MeDef, NetStatsDef], (peers, res) => {
...
export function initNetAckUpdateSystem() {
    ...
    EM.addSystem("netAck", Phase.NETWORK, [PeerDef, InboxDef], [TimeDef, MeDef], ack);
}
