...
class Net {
    ...
    constructor(peerName) {
        if (VERBOSE_NET_LOG)
            console.log(`new Net, peerName: ${peerName}`);
        this.peer = new Peer(peerName);
        this.peer.onopen = (address) => {
            if (VERBOSE_NET_LOG)
                console.log(`peer onopen, address: ${address}`);
    ...
    peerConnected(address, chan) {
        if (VERBOSE_NET_LOG)
            console.log(`NewConnection w/ ${address}`);
    ...
    awaitConnections() {
        this.peer.onconnection = (address, channel) => {
            if (VERBOSE_NET_LOG)
                console.log(`incoming connnection, address:${address}, channel.id:${channel.id}`);
...
export function initNetSystems() {
    ...
    if (VERBOSE_NET_LOG)
        console.log(`added NetStatsDef`);
    EM.addResource(EventsFromNetworkDef);
    if (VERBOSE_NET_LOG)
        console.log(`added EventsFromNetworkDef`);
    EM.addResource(EventsToNetworkDef);
    if (VERBOSE_NET_LOG)
        console.log(`added EventsToNetworkDef`);
    EM.addSystem("getStatsFromNet", Phase.NETWORK, null, [NetStatsDef], getStatsFromNet(net));
    EM.addSystem("getEventsFromNet", Phase.NETWORK, null, [EventsFromNetworkDef], getEventsFromNet(net));
    EM.addSystem("sendEventsToNet", Phase.NETWORK, null, [EventsToNetworkDef], sendEventsToNet(net));
}
