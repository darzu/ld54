import { BallMesh, CubeMesh, CubeRaftMesh, HexMesh, } from "../meshes/mesh-list.js";
...
const mpMeshes = XY.defineMeshSetResource("mp_meshes", CubeMesh, HexMesh, BallMesh, CubeRaftMesh);
const { MpPlayerLocalDef, MpPlayerPropsDef, createMpPlayer, createMpPlayerNow, } = defineNetEntityHelper({
    name: "mpPlayer",
    defaultProps: () => {
        return {
            location: V(0, 0, 0),
    ...
    updateProps: (p, location, color, parentId) => {
        // console.log(
...
const raiseSetLevel = eventWizard("mp-set-level", [], (_, levelIdx) => setLevelLocal(levelIdx), {
...
export async function initMPGame() {
    ...
    if (me.host) {
        createMpRaft();
        EM.addSystem("movePlatform", Phase.GAME_WORLD, [MpRaftPropsDef, PositionDef, RotationDef], [TimeDef], (es, res) => {
            if (es.length !== 1)
                return;
            const platform = es[0];
...
//# sourceMappingURL=game-multiplayer.js.map