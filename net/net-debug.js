...
export function initNetDebugSystem() {
    ...
    EM.addSystem("netDebugToggle", Phase.NETWORK, null, [InputsDef, NetDebugStateDef], (objs, res) => {
        if (res.inputs.keyClicks["6"])
            res.netDebugState.dbgAuthority = !res.netDebugState.dbgAuthority;
    });
    EM.addSystem("netDebugSystem", Phase.NETWORK, [AuthorityDef, RenderableDef], [InputsDef, NetDebugStateDef], (objs, res) => {
        for (const o of objs) {
