...
import { vec3, quat, mat3, V } from "../matrix/sprig-matrix.js";
...
export function bezierSplineFromPoints(points, smoothness) {
    ...
    for (let i = 1; i < points.length - 2; i++) {
        ...
        const startCont = vec3.add(start, vec3.scale(fromPrev, smoothness), vec3.create());
        // const toNext = vec3.normalize(vec3.sub(end, next));
        const toNext = vec3.normalize(vec3.sub(start, next));
        const endCont = vec3.add(end, vec3.scale(toNext, smoothness), vec3.create());
