import { vec2, vec3, quat, mat4, V } from "../matrix/sprig-matrix.js";
...
export function randNormalVec3(out) {
    const res = vec3.set(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5, out ?? vec3.tmp());
...
export function randQuat(out) {
    return quat.fromEuler(Math.random() * Math.PI * 2, Math.random() * Math.PI * 2, Math.random() * Math.PI * 2, out ?? quat.tmp());
}
// matrix utilities
