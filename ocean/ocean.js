...
export const UVPosDef = EM.defineComponent("uvPos", () => vec2.create(), (p, uv) => (uv ? vec2.copy(p, uv) : p));
EM.registerSerializerPair(UVPosDef, (o, buf) => buf.writeVec2(o), (o, buf) => buf.readVec2(o));
export const UVDirDef = EM.defineComponent("uvDir", () => V(0, 1), (p, dir) => (dir ? vec2.copy(p, dir) : p));
...
export async function initOcean(oceanMesh, color) {
    ...
    const uvToGerstnerDispAndNorm = (outDisp, outNorm, uv) => {
        ...
        const disp = vec3.add(vec3.scale(perp, outDisp[0], __temp6), vec3.add(vec3.scale(norm, outDisp[1], __temp7), vec3.scale(tang, outDisp[2], __temp8), __temp11), __temp9);
        ...
        const gNorm = vec3.add(vec3.scale(perp, outNorm[0], __temp6), vec3.add(vec3.scale(norm, outNorm[1], __temp7), vec3.scale(tang, outNorm[2], __temp8), __temp11), __temp10);
