    ...
    const speed = EM.addSystem("controlSpaceSuit", Phase.GAME_PLAYERS, [SpaceSuitDef, RotationDef, LinearVelocityDef], [InputsDef, TimeDef], (suits, res) => {
        for (let e of suits) {
            ...
            if (res.inputs.keyDowns["c"])
                e.spaceSuit.localAccel[1] -= speed;
            const rotatedAccel = vec3.transformQuat(e.spaceSuit.localAccel, e.rotation);
            // change dampen?
            ...
            // camera rotation
            quat.rotateY(e.rotation, -res.inputs.mouseMov[0] * e.spaceSuit.turnSpeed, e.rotation);
            quat.rotateX(e.rotation, -res.inputs.mouseMov[1] * e.spaceSuit.turnSpeed, e.rotation);
            let rollSpeed = 0;
