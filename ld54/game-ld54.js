import { BallMesh, CubeMesh, CubeRaftMesh, HexMesh, LD54AstronautMesh, } from "../meshes/mesh-list.js";
...
import { BubbleDef, LD54GameStateDef, BreathingPlayerDef, SHIP_SPEED, } from "./gamestate.js";
...
const { PlayerLocalDef, PlayerPropsDef, createPlayer, createPlayerNow, createPlayerAsync, } = defineNetEntityHelper({
    ...
    build: (e, res) => {
        console.log(`creating player (${e.id}) auth.pid:${e.authority.pid} me.pid:${res.me.pid}`);
        ...
        EM.set(e, RenderableConstructDef, res.ld54_meshes.cube.proto, RENDER_TRUTH_CUBE);
...
const raiseSetLevel = eventWizard("set-level", [], (_, levelIdx) => setLevelLocal(levelIdx), {
...
export async function initLD54() {
    ...
    if (me.host) {
        createRaft();
        EM.addSystem("movePlatform", Phase.GAME_WORLD, [RaftPropsDef, RaftLocalDef, PositionDef, RotationDef], [TimeDef, PartyDef, LD54GameStateDef], (es, res) => {
            if (es.length !== 1)
                return;
            const raft = es[0];
    ...
    const bubble = EM.new();
    EM.set(bubble, RenderableConstructDef, bubbleMesh, undefined, undefined, BUBBLE_MASK);
