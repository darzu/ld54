...
(function (GameStatus) {
    ...
    GameStatus[GameStatus["Defeat"] = 1] = "Defeat";
    GameStatus[GameStatus["Victory"] = 2] = "Victory";
...
EM.addEagerInit([], [LD54GameStateDef], [], () => {
    EM.addSystem("updateLD54GameState", Phase.PRE_GAME_WORLD, [BreathingPlayerDef, PositionDef], [LD54GameStateDef, PartyDef, TimeDef, TextDef, RendererDef], (es, res) => {
        ...
        res.ld54GameState.bubbleRadius = Math.pow(res.ld54GameState.oxygen / OXYGEN_DENSITY, 1 / 3);
        ...
        if (player) {
            ...
            res.renderer.renderer.updateScene({
                vignetteIntensity: 1 - res.ld54GameState.playerOxygen / STARTING_PLAYER_OXYGEN,
            });
        }
        // have we reached the end of the path and won?
        const path = EM.filterEntities([SpacePathDef])[0];
        if (path) {
            const distanceToEnd = vec3.dist(res.party.pos, path.spacePath.path[path.spacePath.path.length - 1].pos);
            if (distanceToEnd <= 0.1) {
                res.ld54GameState.status = GameStatus.Victory;
            }
        }
        res.text.upperText = `player o2: ${res.ld54GameState.playerOxygen.toFixed(0)} | ship o2: ${res.ld54GameState.oxygen.toFixed(0)} fuel: ${res.ld54GameState.fuel.toFixed(0)}`;
    });
    EM.addSystem("updateBubble", Phase.PRE_GAME_PLAYERS, [BubbleDef], [LD54GameStateDef, RendererDef, TimeDef], (es, res) => {
