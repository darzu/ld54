...
import { mat3, mat4, quat, V, vec3 } from "../matrix/sprig-matrix.js";
...
import { GrassCutTexPtr, grassPoolPtr, registerUploadGrassData, renderGrassPipe, } from "./std-grass.js";
...
const grassGameMeshesDef = XY.defineMeshSetResource("gg_meshes", UnitCubeMesh, GizmoMesh, BallMesh);
...
export async function initGrassGame(hosting) {
    ...
    const res = await EM.whenResources(grassGameMeshesDef, 
    // WoodAssetsDef,
    // GlobalCursor3dDef,
    ...
    EM.addSystem("grassGameRenderPipelines", Phase.GAME_WORLD, null, [RendererDef, DevConsoleDef], (_, res) => {
        // renderer
    ...
    transformMesh(groundMesh, mat4.fromScaling(V(WORLD_HEIGHT, 1.0, WORLD_WIDTH)));
    ...
    const gr = EM.new();
    EM.set(gr, RenderableConstructDef, grMesh, undefined, 
    // false,
    ...
    // update grass
    EM.addSystem("updateGrass", Phase.GAME_WORLD, [HsPlayerDef, CameraFollowDef, WorldFrameDef], [], (es, res) => {
        const player = es[0];
    ...
    EM.addSystem("cuttingOnOff", Phase.GAME_PLAYERS, null, [InputsDef], (_, res) => {
        // TODO(@darzu):
    ...
    EM.addSystem("cutGrassUnderShip", Phase.GAME_WORLD, [ShipDef, PositionDef, WorldFrameDef, PhysicsStateDef], [PartyDef, ScoreDef], (es, res) => {
        if (!es.length)
            return;
        const ship = es[0];
    ...
    EM.addSystem("furlUnfurl", Phase.GAME_PLAYERS, null, [InputsDef], (_, res) => {
...
//# sourceMappingURL=game-grass.js.map