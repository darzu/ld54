...
export const LocalPlayerEntityDef = EM.defineResource("localPlayerEnt", (playerId) => ({
    playerId: playerId || 0,
}));
export const PlayerHsPropsDef = defineSerializableComponent("hsPlayerProps", () => ({ location: vec3.create() }), (p, loc) => {
    if (loc)
        vec3.copy(p.location, loc);
    return p;
}, (c, writer) => {
...
EM.addEagerInit([PlayerHsPropsDef], [], [], () => {
    EM.addSystem("buildHsPlayers", Phase.PRE_GAME_WORLD, [PlayerHsPropsDef], [MeDef, AllMeshesDef], (players, res) => {
        for (let e of players) {
            if (FinishedDef.isOn(e))
                continue;
            const props = e.hsPlayerProps;
            EM.set(e, PositionDef, props.location);
            EM.set(e, RotationDef, quat.rotateY(quat.IDENTITY, Math.PI, quat.create()));
    ...
    EM.addSystem("hsPlayerFacingDir", Phase.GAME_PLAYERS, [HsPlayerDef, WorldFrameDef], [GlobalCursor3dDef], (players, res) => {
    ...
    EM.addSystem("stepHsPlayers", Phase.GAME_PLAYERS, [
        ...
        ControllableDef,
        CanManDef,
    ], [TimeDef, CameraDef, InputsDef, MeDef, PhysicsResultsDef], (players, res) => {
        ...
        for (let p of players) {
            ...
            if (cheat && inputs.keyClicks["t"]) {
                const targetId = EM.getResource(GlobalCursor3dDef)?.cursor()?.cursor3d.hitId;
                if (targetId) {
...
//# sourceMappingURL=hs-player.js.map