...
import { endGame, HyperspaceGameState, HSGameStateDef, startGame, } from "./hyperspace-gamestate.js";
...
export const ShipPartDef = EM.defineNonupdatableComponent("shipPart", (critical) => ({
    critical,
    damaged: false,
}));
export const { RudderPropsDef, RudderLocalDef, createRudderNow } = defineNetEntityHelper({
    name: "rudder",
    defaultProps: () => ({
        shipId: 0,
    }),
    updateProps: (p, shipId) => Object.assign(p, {
        shipId: shipId ?? 0,
    }),
...
// hyperspace ship
export const { HsShipPropsDef, HsShipLocalDef, createHsShip } = defineNetEntityHelper({
    name: "hsShip",
    defaultProps: () => ({
    ...
    build: async (s, res) => {
        ...
        for (let i = 0; i < res.allMeshes.ship_broken.length; i++) {
            ...
            s.hsShipLocal.parts.push(createRef(part.id, [ShipPartDef, RenderableDef]));
        }
        makeOrrery(s.id);
...
export function registerShipSystems() {
    EM.addSystem("startGame", Phase.GAME_WORLD, [GemPropsDef, InRangeDef], [HSGameStateDef, PhysicsResultsDef, MeDef, InputsDef, LocalPlayerEntityDef], (gems, res) => {
        for (let gem of gems) {
            if (DeletedDef.isOn(gem))
                continue;
            if (res.hsGameState.state !== HyperspaceGameState.LOBBY)
                continue;
    ...
    const raiseShipHit = eventWizard("ship-hit", [[HsShipLocalDef]], ([ship], partIdx) => {
    ...
    EM.addSystem("shipHealthCheck", Phase.GAME_WORLD, [HsShipPropsDef, HsShipLocalDef, PositionDef, AuthorityDef], [
        AudioDef,
        InputsDef,
        CameraDef,
        HSGameStateDef,
    ...
    EM.addSystem("playerShipMove", Phase.GAME_PLAYERS, [
        ...
        // RotationDef,
        UVDirDef,
    ], [HSGameStateDef, MeDef, InputsDef, DevConsoleDef], (ships, res) => {
        if (res.hsGameState.state !== HyperspaceGameState.PLAYING) {
            return;
        }
        for (let s of ships) {
    ...
    EM.addSystem("shipUpdateParty", Phase.GAME_WORLD, [HsShipLocalDef, HsShipPropsDef, PositionDef], [PartyDef], (ships, res) => {
        if (ships[0])
            vec3.copy(res.party.pos, ships[0].position);
    });
    // If a rudder isn't being manned, smooth it back towards straight
    EM.addSystem("easeRudder", Phase.GAME_WORLD, [RudderPropsDef, TurretDef, YawPitchDef, AuthorityDef], [MeDef], (rudders, res) => {
...
//# sourceMappingURL=hyperspace-ship.js.map