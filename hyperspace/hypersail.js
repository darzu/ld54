...
const SailColorDef = EM.defineComponent("sailColor", () => vec3.create(), (p, color) => (color ? vec3.copy(p, color) : p));
// TODO: we need warnings if you forget to call the buildProps system!
export const { HypMastPropsDef, HypMastLocalDef, createHypMastNow } = defineNetEntityHelper({
    name: "hypMast",
    defaultProps: () => ({
        shipId: 0,
    }),
...
export function registerHypersailSystems() {
    EM.addSystem("updateMastBoom", Phase.GAME_PLAYERS, [HypMastPropsDef, HypMastLocalDef, TurretDef, BoomPitchesDef], [InputsDef, RendererDef], (masts, res) => {
    ...
    EM.addSystem("sail", Phase.GAME_PLAYERS, [HsShipPropsDef, UVShipDef, WorldFrameDef, AuthorityDef], [MeDef, HSGameStateDef], (es, res) => {
