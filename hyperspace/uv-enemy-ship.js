...
import { vec2, vec3, quat, V } from "../matrix/sprig-matrix.js";
...
export function createEnemyCrew(allMeshes, parent, pos) {
    const e = EM.new();
...
export const { EnemyShipPropsDef, EnemyShipLocalDef, createEnemyShip } = defineNetEntityHelper({
    name: "enemyShip",
    defaultProps: () => {
        return {
    ...
    updateProps: (p, uvLoc, speed, wheelSpeed, uvDir, parent) => {
        if (uvLoc)
            vec2.copy(p.uvLoc, uvLoc);
        if (speed !== undefined)
            p.speed = speed;
        if (wheelSpeed !== undefined)
            p.wheelSpeed = wheelSpeed;
        if (uvDir)
            vec2.copy(p.uvDir, uvDir);
        if (parent !== undefined)
            p.parent = parent;
        return p;
    },
...
export function registerEnemyShipSystems() {
    EM.addSystem("stepEnemyShips", Phase.GAME_WORLD, [EnemyShipLocalDef, EnemyShipPropsDef, UVDirDef, UVShipDef, AuthorityDef], [TimeDef, MeDef], (enemyShips, res) => {
    ...
    EM.addSystem("enemyShipsFire", Phase.GAME_WORLD, [EnemyShipLocalDef, AuthorityDef], [TimeDef, MeDef, PhysicsResultsDef], (enemyShips, res) => {
        for (let o of enemyShips) {
            if (o.authority.pid !== res.me.pid)
                continue;
            // TODO(@darzu): COUNT DOWN FIREZONE
            const hits = res.physicsResults.collidesWith.get(o.enemyShipLocal.fireZoneId);
            const seesPlayer = hits?.some((h) => !!EM.findEntity(h, [HsShipLocalDef]));
            if (seesPlayer) {
            ...
            if (o.enemyShipLocal.fireDelay < 0) {
                ...
                if (cannon) {
                    ...
                    fireBullet(2, cannon.world.position, cannon.world.rotation, bulletSpeed, 
    ...
    EM.addSystem("breakEnemyShips", Phase.GAME_WORLD, [EnemyShipLocalDef, PositionDef, RotationDef], [PhysicsResultsDef, AllMeshesDef, AudioDef, MeDef, DetectedEventsDef], (objs, res) => {
...
export function breakEnemyShip(enemyShip, enemyShipParts, music) {
...
//# sourceMappingURL=uv-enemy-ship.js.map