...
import { gjk, penetrationDepth } from "./narrowphase.js";
...
*/
let __frame = 0;
export async function initGJKSandbox(hosting) {
    ...
    EM.addSystem("checkGJK", Phase.GAME_WORLD, null, [InputsDef], (_, { inputs }) => {
        // console.log(__frame);
        // __frame++;
        // if (!inputs.keyClicks["g"]) return;
        // TODO(@darzu):
        let playerShape = createWorldShape(res.allMeshes.cube, b2.position, b2.rotation, lastPlayerPos);
        const gameMeshes = [
            res.allMeshes.cube,
            res.allMeshes.ball,
            res.allMeshes.tetra,
        ...
        for (let i = 0; i < ents.length; i++) {
            ...
            if (simplex &&
                ...
                playerShape = createWorldShape(res.allMeshes.cube, b2.position, b2.rotation, lastPlayerPos);
