...
export const DbgMeshDef = EM.defineNonupdatableComponent("_physDbgMesh", (colliderId) => {
    return {
...
export function registerPhysicsDebuggerSystem() {
    ...
    // add collider meshes
    EM.addSystem("dbgColliderMeshes", Phase.POST_PHYSICS, [PhysicsStateDef], [PhysicsDbgDef, AllMeshesDef], (es, res) => {
        for (let e of es) {
            if (!res._physDbgState.colliderMeshes.has(e.id)) {
                for (let c of e._phys.colliders) {
                    ...
                    EM.set(dbgE, RenderableConstructDef, res.allMeshes.wireCube.proto, res._physDbgState.showAABBs, 1);
                    // colored
    ...
    EM.addSystem("debugMeshes", Phase.POST_PHYSICS, [DbgMeshDef, RenderableDef], [InputsDef, PhysicsDbgDef], (es, res) => {
    ...
    EM.addSystem("debugMeshTransform", Phase.POST_PHYSICS, [DbgMeshDef, WorldFrameDef, ...LocalFrameDefs], [PhysicsBroadCollidersDef], (es, res) => {
        for (let e of es) {
