...
export const TeleportDef = EM.defineComponent("teleport", () => true, (_) => true);
EM.addEagerInit([TeleportDef, ColliderDef], [], [], () => {
    EM.addSystem("teleportSet", Phase.PRE_PHYSICS, [TeleportDef, ColliderDef], [], (cs) => {
        for (let c of cs) {
            if (!c.collider.solid) {
    ...
    EM.addSystem("teleportUnset", Phase.POST_PHYSICS, [TeleportDef, ColliderDef], [], (cs) => {
        for (let c of cs) {
