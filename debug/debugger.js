...
import { EM, initFnToString, } from "../ecs/entity-manager.js";
import { MetaPhases, NameFromPhase, PhaseNameList, PhaseNameToMetaPhase, } from "../ecs/sys-phase.js";
...
function mkDbgEnt(id) {
    if (id === 0)
        throw `Invalid entity id '0', use resources!`;
...
g.cameraFollow.pitchOffset = ${target.cameraFollow.pitchOffset.toFixed(3)};
    ...
    summarizeInit: () => {
        const inits = [...EM.initFnMsStats.keys()].map((id) => EM.allInits.get(id));
        const initsAndTimes = inits.map((reg) => [reg, EM.initFnMsStats.get(reg.id)]);
    ...
    summarizeStats: () => {
        ...
        const phaseTimes = toMap(PhaseNameList, (n) => n, (_) => 0);
        for (let s of Object.keys(stats)) {
        ...
        const metaPhaseTimes = toMap(MetaPhases, (n) => n, (_) => 0);
        for (let p of phaseTimes.keys()) {
