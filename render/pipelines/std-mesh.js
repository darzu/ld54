import { ColorDef, TintsDef, applyTints, AlphaDef, } from "../../color/color-ecs.js";
...
EM.addEagerInit([RenderDataStdDef], [], [], () => {
    ...
    EM.addSystem("stdRenderListDeadHidden", Phase.RENDER_PRE_DRAW, [RendererWorldFrameDef, RenderableDef, RenderDataStdDef, DeadDef], [], (objs, _) => {
        for (let o of objs)
            if (o.renderable.enabled && o.renderable.hidden && !DeletedDef.isOn(o))
                renderObjs.push(o);
    });
    EM.addSystem("stdRenderList", Phase.RENDER_PRE_DRAW, [RendererWorldFrameDef, RenderableDef, RenderDataStdDef], [], (objs, _) => {
        for (let o of objs)
            if (o.renderable.enabled && !DeletedDef.isOn(o))
                renderObjs.push(o);
    });
    EM.addSystem("stdRenderableDataUpdate", Phase.RENDER_PRE_DRAW, null, [RendererDef], (_, res) => {
        for (let o of renderObjs) {
            if (updateStdRenderData(o)) {
                o.renderable.meshHandle.pool.updateUniform(o.renderable.meshHandle, o.renderDataStd);
            }
        }
        renderObjs.length = 0;
    });
