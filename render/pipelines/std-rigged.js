import { vec3, vec4, V } from "../../matrix/sprig-matrix.js";
...
import { isRigged, } from "../../meshes/mesh.js";
import { computeUniData, mainDepthTex, MeshUniformStruct, positionsTexturePtr, RenderDataStdDef, sceneBufPtr, surfacesTexturePtr, unlitTexturePtr, worldNormsAndFresTexPtr, } from "./std-scene.js";
...
function isRiggedMeshWithJointIdx(m) {
...
const VertexStruct = createCyStruct({
    ...
    isCompact: true,
    serializer: ({ jointIds, jointWeights, position, color, normal, uv, surfaceId }, _, offsets_32, views) => {
...
const tempVertsData = [];
function computeVertsData(m, startIdx, count) {
...
export function createRiggedMeshPool(renderer) {
    ...
    function updateJointMatrices(handle, mats) {
        joints.queueUpdates(
        // TODO: avoid unnecessary object allocation here--this should
        // just be an array of mat4s
        mats.map((m) => ({
            jointMatrix: m,
        })), handle.jointIdx, 0, mats.length);
    }
    return { addRiggedMesh, updateJointMatrices, ptr: poolPtr };
}
export const stdRiggedRenderPipeline = CY.createRenderPipeline("stdRiggedRender", {
...
//# sourceMappingURL=std-rigged.js.map