...
// https://people.cs.clemson.edu/~dhouse/courses/405/docs/brief-obj-file-format.html
// http://paulbourke.net/dataformats/obj/
import { vec2, vec3, vec4, V } from "../matrix/sprig-matrix.js";
import { assert } from "../utils/util.js";
import { idPair, isString } from "../utils/util.js";
...
function parseVec(p, len) {
    ...
    if (nums.length !== len)
        return `invalid vector-${len} format: ${p.join(" ")}`;
    if (len === 2)
        return vec2.clone(nums);
    else if (len === 3)
        return vec3.clone(nums);
    else
        assert(false);
}
...
`;
//# sourceMappingURL=import-obj.js.map