...
function registerObjClicker() {
    ...
    EM.addSystem("modelerOnOff", Phase.GAME_PLAYERS, null, [ModelerDef, InputsDef, CanvasDef], (_, res) => {
    ...
    EM.addSystem("modelerClicks", Phase.GAME_PLAYERS, null, [ModelerDef, CameraComputedDef, InputsDef, PhysicsResultsDef], (_, res) => {
        if (!res.modeler.clickerEnabled)
            return;
...
function registerAABBBuilder() {
    EM.addSystem("aabbBuilder", Phase.GAME_PLAYERS, null, [InputsDef, ModelerDef, AllMeshesDef], (_, res) => {
        // create a new box
