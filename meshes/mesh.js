...
import { arrayUnsortedEqual } from "../utils/util.js";
import { vec3Dbg2, vec4Dbg2 } from "../utils/utils-3d.js";
import { drawBall } from "../utils/utils-game.js";
export function isRigged(mesh) {
    return !!mesh.rigging;
}
...
export function validateMesh(m) {
    ...
    if (m.rigging) {
        ...
        assert(rigging.jointIds.length === vertCount, `mesh ${dbgName} joint IDs count (${rigging.jointIds.length}) doesn't match vert count (${vertCount}`);
        assert(rigging.jointWeights.length === vertCount, `mesh ${dbgName} joint IDs count (${rigging.jointWeights.length}) doesn't match vert count (${vertCount}`);
        const numJoints = rigging.parents.length;
        for (let ids of rigging.jointIds) {
            for (let ji of ids) {
                assert(0 <= ji && ji < numJoints, `invalid joint ID ${ji} in mesh ${dbgName}`);
            }
        }
        for (let p of rigging.parents) {
            assert(0 <= p && p < numJoints, `invalid joint ${p} as parent in mesh ${dbgName}`);
        }
    }
}
...
export function unshareProvokingVerticesWithMap(input) {
    ...
    const normals = input.normals
        ? [...input.normals]
        : undefined;
    const jointIds = input.rigging
        ? [...input.rigging.jointIds]
        : undefined;
    const jointWeights = input.rigging
    ...
    input.tri.forEach(([i0, i1, i2]) => {
        if (!provoking[i0]) {
            ...
            if (jointIds)
                jointIds.push(input.rigging.jointIds[i0]);
            if (jointWeights)
                jointWeights.push(input.rigging.jointWeights[i0]);
            provoking[i3] = true;
            tri.push(V(i3, i1, i2));
        }
    });
