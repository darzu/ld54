...
export function createSail(width, height, scale) {
    const ent = EM.new();
    EM.set(ent, SailDef);
...
EM.addSystem("applyWindToSail", Phase.GAME_WORLD, [SailDef, WorldFrameDef], [WindDef], (es, res) => {
    for (let e of es) {
...
EM.addSystem("billow", Phase.GAME_WORLD, [SailDef, RenderableDef], [RendererDef], (es, { renderer }) => {
    assert(es.length <= 1);
    if (!es.length)
        return;
    const e = es[0];
...
export function createMast(res) {
...
EM.addSystem("mastForce", Phase.GAME_WORLD, [MastDef, RotationDef], [], (es) => {
    for (let e of es) {
        const sail = e.mast.sail().sail;
...
// UNUSED:
function useWindToTurn(us, targetPos, windDir, outSailRot) {
    // TODO(@darzu): expensive
    const fwd = tV(0, 0, -1);
    const behind = tV(0, 0, 1);
    const angleToParty = angleBetweenPosXZ(us.position, us.rotation, fwd, targetPos);
    // turn the tower
