...
import { cloneMesh, scaleMesh, stringifyMesh } from "../meshes/mesh.js";
...
const PANEL_W = 4 * 12;
const PANEL_H = 3 * 12;
export const UICursorDef = EM.defineResource("uiCursor", (cursor) => ({
    cursor,
}));
...
export async function initFontEditor() {
    ...
    const res = await EM.whenResources(AllMeshesDef, RendererDef, ButtonsStateDef);
    ...
    EM.addSystem("uiCameraView", Phase.GAME_WORLD, null, [CameraComputedDef, CanvasDef, CameraDef, InputsDef, UICursorDef], async (_, res) => {
    ...
    // Edit letters
    EM.addSystem("letterBtnClick", Phase.GAME_WORLD, null, [ButtonsStateDef, MeshEditorDef, TextDef], (_, res) => {
