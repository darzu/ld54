...
import { vec3, quat, mat4, mat3, V, } from "../matrix/sprig-matrix.js";
import { extrudeQuad, meshToHalfEdgePoly, } from "../meshes/half-edge.js";
...
//  with pools?
const HEdgeDef = EM.defineNonupdatableComponent("hedge", (he) => ({
    he,
}));
const HVertDef = EM.defineNonupdatableComponent("hvert", (hv) => ({
    hv,
}));
export const MeshEditorDef = EM.defineResource("meshEditor", (me) => me);
...
async function createMeshEditor() {
    ...
    async function setMesh(handle) {
        ...
        if (res.hpEnt) {
            ...
            const hpEnt_ = EM.new();
            EM.set(hpEnt_, RenderableConstructDef, handle, true, undefined, undefined, meshPoolPtr, false);
...
export async function initMeshEditor() {
    ...
    EM.addSystem("editHPoly", Phase.GAME_WORLD, null, [MeshEditorDef, RendererDef, ButtonsStateDef, WidgetLayerDef], (_, { meshEditor: e, renderer, buttonsState, widgets }) => {
