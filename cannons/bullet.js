...
export const BulletConstructDef = EM.defineComponent("bulletConstruct", () => {
    return {
        location: V(0, 0, 0),
...
}, (p, loc, vel, angVel, team, gravity, health) => {
    if (loc)
        vec3.copy(p.location, loc);
    if (vel)
        vec3.copy(p.linearVelocity, vel);
    if (angVel)
        vec3.copy(p.angularVelocity, angVel);
    if (team !== undefined)
        p.team = team;
...
export function createOrResetBullet(e, res) {
...
export function registerBuildBulletsSystem() {
    EM.addSystem("buildBullets", Phase.GAME_WORLD, [BulletConstructDef], [MeDef, AllMeshesDef], (bullets, res) => {
...
export function registerBulletUpdate() {
    // TODO(@darzu): remove?
    EM.addSystem("updateBullets", Phase.GAME_WORLD, [BulletConstructDef, BulletDef, PositionDef, LinearVelocityDef], [TimeDef], (bullets, res) => {
        // for (let b of bullets) {
        //   b.linearVelocity[1] -=
...
export async function fireBullet(team, location, rotation, speed, // = 0.02,
rotationSpeed, // = 0.02,
gravity, // = 6
health, bulletAxis) {
    {
