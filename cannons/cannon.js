...
export const { CannonPropsDef, CannonLocalDef, createCannon, createCannonNow } = defineNetEntityHelper({
    name: "cannon",
    defaultProps: () => {
        return {
            location: V(0, 0, 0),
    ...
    updateProps: (p, location, yaw, pitch, parentId) => {
        if (location)
    ...
    build: (e, res) => {
        ...
        constructNetTurret(e, props.yaw, props.pitch, res.mesh_ld51_cannon.aabb, 0, undefined, undefined, undefined, true, 1.0, Math.PI / 4, "W/S: pitch, A/D: turn, left click: fire, E: drop cannon");
        EM.set(e, ColorDef, V(0, 0, 0));
        EM.set(e, RenderableConstructDef, res.mesh_ld51_cannon.mesh // TODO(@darzu): PERF: use .proto?
        );
...
EM.addEagerInit([CannonPropsDef], [], [], () => {
    EM.addSystem("reloadCannon", Phase.GAME_WORLD, [CannonLocalDef], [TimeDef], (cannons, res) => {
    ...
    const raiseFireCannon = eventWizard("fire-cannon", [[CanManDef], [CannonLocalDef, WorldFrameDef]], ([player, cannon]) => {
        ...
        if (player.id === EM.getResource(LocalPlayerEntityDef)?.playerId) {
            ...
            const v = 0.18;
            const g = 6.0 * 0.00001;
            const b = fireBullet(2, firePos, fireDir, v, 0.02, g, 
            // 2.0,
            20.0, [0, 0, -1]);
        }
    ...
    EM.addSystem("playerControlCannon", Phase.GAME_PLAYERS, [CannonLocalDef, TurretDef, WorldFrameDef], [InputsDef, LocalPlayerEntityDef], (cannons, res) => {
    ...
    EM.addSystem("playerManCanon", Phase.GAME_PLAYERS, [CannonLocalDef, TurretDef, InRangeDef, AuthorityDef, WorldFrameDef], [DetectedEventsDef, InputsDef, LocalPlayerEntityDef], (cannons, res) => {
