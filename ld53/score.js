...
EM.addSystem("updateScoreDisplay", Phase.POST_GAME_WORLD, [ShipHealthDef], [ScoreDef, TextDef, CanvasDef], (es, res) => {
    const ship = es[0];
    if (!ship)
        return;
...
EM.addSystem("detectGameEnd", Phase.POST_GAME_WORLD, [ShipHealthDef], [ScoreDef, TextDef, TimeDef, PartyDef, HostDef], async (es, res) => {
    const ship = es[0];
    if (!ship)
        return;
    ...
    if (res.score.pendingTransition)
        return;
    // relevant facts
    const shipDead = ship.shipHealth.health <= 0;
    const shipInEndZone = pointInAABB(res.score.endZone()._phys.colliders[0].aabb, res.party.pos);
    const alreadyCompletedLevel = res.score.completedLevels.has(res.score.levelNumber);
    // game lost
    if (shipDead) {
